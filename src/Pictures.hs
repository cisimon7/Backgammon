...
drawDice _player ((d1, d2),(d3, d4)) = wCubesRendered <> rCubesRendered
    where
        ...
        renderFocusedCubes testPawn = case testPawn of
            (PawnWhite _ _) -> translate (3*cSize) 0 (fCube white (x d1)) 
                               <> translate (5*cSize) 0 (fCube white (x d2))
            (PawnRed _ _)   -> translate (-3*cSize) 0 (fCube redChipColor (x d3)) 
                               <> translate (-5*cSize) 0 (fCube redChipColor (x d4))
  
        justCubes :: Pawn -> Picture
        justCubes testPawn = case testPawn of
            (PawnWhite _ _) -> translate (3*cSize) 0 (cube white (x d1)) 
                               <> translate (5*cSize) 0 (cube white (x d2))
            (PawnRed _ _)   -> translate (-3*cSize) 0 (cube redChipColor (x d3)) 
                               <> translate (-5*cSize) 0 (cube redChipColor (x d4))
      ...
      where
          ...
          bChipRender chip = scale 0.7 0.7
                   ...
                   where h = case chip of
                               ...
                               (PawnRed _ _)   ->  (fromIntegral screenHeight) + gap - 1.5*pawnRadius - 2*pawnRadius*fromIntegral (pt chip)
